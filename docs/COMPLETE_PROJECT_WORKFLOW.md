...
## 標題來源優先順序與 redirect 對應表

為了在標題經人工微調後仍能穩定維護 redirect，
本專案對「標題來源」與「新舊網址對應」有以下設計：

### 標題解析優先順序

1. **首頁主選單 / 首頁下方 sitemap**  
   - 先由爬蟲抽出首頁主選單與簡易 sitemap，形成一份 `menu_sitemap_index`。  
   - 若某 URL 在此索引中有明確 `label`，則優先採用該文字作為該區段／頁面的標題來源。

2. **HTML `<title>` 與內容 heuristics**  
   - 若 menu/sitemap 中找不到對應：  
     - 先嘗試由 `<head><title>` 中抽出合適的主標（去除站名、重複字串）。  
     - 再依 HTML 結構（H1、`heading3`、`T20` 等）進行 heuristics 判斷。

3. **人工 overrides（少量例外）**  
   - 對於仍無法自動判斷、但又十分重要的頁面，
     可在 `title_overrides.json` 中手動指定最終標題。

### Redirect 對應表與標題／slug 的關係

- 每篇內容會有穩定的 `external_id`（以及 WordPress `post_id`）：
  - 這兩者不因標題調整而改變。  
  - redirect 對應表的生成，應以 `external_id`／`post_id` 為基礎。

- 建議流程：
  1. 匯入 WordPress 後，由 WP-CLI 撰寫 `ct_generate_redirects` 類指令：  
     - 讀取每篇文章的 `legacy_urls[]`（舊站 URL）。  
     - 使用 `get_permalink($post_id)` 取得**目前**的 permalink。  
     - 產出 `legacy_url → current_permalink` 的對應表。  
  2. 若上線前調整了標題或 slug：  
     - 無需手動編輯對應表，只需重新執行 `ct_generate_redirects`，
       讓對應表自動與最新 permalink 對齊。

- 如此一來：
  - 「標題／slug 調整」與「redirect 維護」解耦，
  - 新舊站對應表被視為「隨時可重算的產物」，而非需要人工長期維護的靜態檔案。
